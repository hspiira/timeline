...
export function validateConnection(
  ...
  if (desc?.isCondition && label == null) return { valid: false, error: 'Condition edges must have label "true" or "false"' }
  if (desc?.isCondition) {
    ...
    if (label === 'true' && hasTrue) return { valid: false, error: 'Condition already has a "true" edge' }
    if (label === 'false' && hasFalse) return { valid: false, error: 'Condition already has a "false" edge' }
  }
