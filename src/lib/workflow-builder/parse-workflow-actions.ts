 ...
 */
export function getStepsFromWorkflowActions(
  actions: unknown
): ParsedWorkflowStep[] {
  ...
  actions.forEach((action: unknown, index: number) => {
    ...
    const tasks: ParsedWorkflowTask[] = tasksRaw.map((t: unknown) => {
      ...
      const name =
        typeof task.name === 'string' ? task.name : 'Task'
    ...
    steps.push({
      ...
      name: stepName,
      description:
        typeof params.stepDescription === 'string'
      ...
      tasks,
      rejectToStepId:
        typeof params.reject_to_step_id === 'string'
          ? params.reject_to_step_id
          : undefined,
    })
  })
