...
class OAuthProviderConfig(AuditedMultiTenantModel, Base):
    ...
    redirect_uri_whitelist: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
    ...
    provider_metadata: Mapped[dict[str, str] | None] = mapped_column(JSON, nullable=True)
    ...
    rate_limit_connections_per_hour: Mapped[int | None] = mapped_column(Integer, nullable=True, default=10)
    ...
    __table_args__ = (
        ...
        Index("ix_oauth_provider_config_active", "tenant_id", "provider_type", "is_active"),
    )
